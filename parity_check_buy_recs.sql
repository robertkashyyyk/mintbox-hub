-- ============================================================================
-- PARITY TEST for the get_buy_recommendations optimization.
-- Run this FIRST in the Supabase SQL editor (project vcfbegjpkvxkqpptyxni).
-- It is 100% NON-DESTRUCTIVE: it swaps in the new function inside a transaction,
-- compares old vs new output, prints the diff, then ROLLS BACK (nothing persists).
--
-- EXPECTED RESULT: in_old_not_new = 0 AND in_new_not_old = 0  → identical output.
-- old_total / new_total should also match (the count of rows the buying page shows).
-- If both diffs are 0, apply migration 20260616190000_buy_recs_candidate_prefilter.sql.
-- ============================================================================
BEGIN;
SET LOCAL statement_timeout = 0;   -- the OLD function is ~12s; the editor has no API timeout

-- 1. Snapshot the CURRENT (old) output across every column that matters.
CREATE TEMP TABLE _old ON COMMIT DROP AS
  SELECT sku, product_name, brand_id, brand_name, supplier_id, supplier_name,
         ...
         required_qty, raw_required_qty, box_quantity,
         pending_po_qty, pending_po_id, sales_4w, status
  FROM get_buy_recommendations(NULL, NULL, true);

-- 2. Swap in the NEW (optimized) function — inside the transaction only.
...
RETURNS TABLE(
  sku text, product_name text, brand_id uuid, brand_name text, supplier_id uuid,
  supplier_name text, current_stock numeric, on_order numeric, back_orders numeric,
  low_stock_alert numeric, unit_cost numeric, required_qty numeric, raw_required_qty numeric,
  box_quantity integer, pending_po_qty numeric, pending_po_id uuid, sales_4w numeric, status text
)
...
AS $function$
  ...
  todays_asn AS (
    SELECT sku, SUM(qty)::numeric AS asn_qty FROM todays_open_asns GROUP BY sku
  ),
  cand AS MATERIALIZED (
    SELECT
      ...
      CASE WHEN COALESCE(pc.low_stock_alert_level, 0) > cfg.lsa_min
           THEN COALESCE(pc.low_stock_alert_level, 0)::numeric ELSE 0::numeric END AS effective_lsa,
    ...
    WHERE COALESCE(pc.quarantined, false) = false
      ...
      AND (
        (
          ...
          + CASE WHEN COALESCE(pc.low_stock_alert_level, 0) > cfg.lsa_min
                 THEN COALESCE(pc.low_stock_alert_level, 0)::numeric ELSE 0::numeric END
        )
        > (
  ...
  base AS (
    SELECT cand.*, b.name AS brand_name, sp.supplier_id AS supplier_id, s.name AS supplier_name
    FROM cand
    LEFT JOIN brands b ON b.id = cand.brand_id
    LEFT JOIN LATERAL (
      SELECT sp2.supplier_id, sp2.prefix
      FROM sku_prefixes sp2
      WHERE upper(cand.sku) LIKE sp2.prefix || '-%' OR upper(cand.sku) LIKE sp2.prefix || '/%'
      ORDER BY length(sp2.prefix) DESC LIMIT 1
    ) sp ON true
    LEFT JOIN suppliers s ON s.id = sp.supplier_id
  ),
  scored AS (
    SELECT *, GREATEST((back_orders + effective_lsa) - (current_stock + on_order), 0) AS raw_required
    FROM base
  ),
  boxed AS (
    SELECT *,
      CASE WHEN raw_required <= 0 THEN raw_required
           WHEN box_quantity <= 1 THEN raw_required
           ELSE (CEIL(raw_required::numeric / box_quantity::numeric) * box_quantity::numeric) END AS required_qty_boxed
    FROM scored
  )
  SELECT sku, product_name, brand_id, brand_name, supplier_id, supplier_name,
    current_stock, on_order, back_orders, low_stock_alert, unit_cost,
    required_qty_boxed AS required_qty, raw_required AS raw_required_qty, box_quantity,
    pending_po_qty, pending_po_id, sales_4w,
    CASE WHEN pending_po_qty > 0 THEN 'po_sent_pending'
         WHEN required_qty_boxed > 0 THEN 'recommended' ELSE 'covered' END AS status
  FROM boxed
...
-- 3. Snapshot the NEW output (same columns).
