...
Deno.serve(async (req) => {
  ...
  // Profit-repricer pushes only — exclude liquidation/clearance cuts, charm-snap
  // tidies, and elasticity tests so none is counted as repricing value.
  const { data: pending } = await admin.from("threeds_reprice_pending")
    .select("store_id, sku, price, queued_at").not("source", "in", "(liquidation,charm_snap,elasticity)");
