// ============================================================================
// amazon-pull-merchant-listings — pulls GET_MERCHANT_LISTINGS_ALL_DATA: every
// listing's seller-SKU, ASIN, and the EAN/UPC (product-id) we listed with.
// Feeds amazon.listings, which gives the ASIN->EAN link the barcode arm of the
// SKU map uses to resolve the pure-FBA residue.
//
// Body: { reportId?: string, pollSeconds?: number }
  ...
  try {
    ...
    while (true) {
      ...
      if (meta.processingStatus === "CANCELLED" || meta.processingStatus === "FATAL")
        return json({ status: meta.processingStatus, reportId, message: "Listings report ended " + meta.processingStatus, meta }, 200);
    ...
    const rows = tsv.map((r) => ({
      seller_sku: r["seller-sku"] ?? "",
      asin: r["asin1"] ?? r["asin"] ?? "",
      ean: r["product-id"] ?? "",
      product_id_type: r["product-id-type"] ?? "",
    ...
    const { data: ing, error: ingErr } = await supa.rpc("amazon_ingest_listings", {
      p_marketplace_id: marketplaceId, p_report_id: reportId, p_document_id: meta.reportDocumentId,
      p_processing_status: meta.processingStatus, p_rows: rows,
    });
    if (ingErr) return json({ error: "ingest RPC failed", detail: ingErr.message, rows: rows.length }, 500);

    return json({ ok: true, reportId, listings: rows.length, withEan, ingest: ing }, 200);
  } catch (e) {
