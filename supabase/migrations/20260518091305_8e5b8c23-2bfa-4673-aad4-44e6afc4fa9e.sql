...
ALTER TABLE public.products_cache
  ADD COLUMN IF NOT EXISTS mintsoft_back_orders numeric;

-- Replace bulk_update_stock_from_sftp to accept on_order and mintsoft_back_orders too.
...
BEGIN
  CREATE TEMP TABLE _stock_in (
    ...
    stock_level numeric,
    on_order numeric,
    mintsoft_back_orders numeric
  ) ON COMMIT DROP;

  INSERT INTO _stock_in (sku, stock_level, on_order, mintsoft_back_orders)
  SELECT DISTINCT ON (x.sku)
    x.sku, x.stock_level, x.on_order, x.mintsoft_back_orders
  ...
  WITH upd AS (
    ...
    SET
      current_stock        = COALESCE(s.stock_level, p.current_stock),
      on_order             = COALESCE(s.on_order, p.on_order),
      mintsoft_back_orders = COALESCE(s.mintsoft_back_orders, p.mintsoft_back_orders),
...
-- Update get_buy_recommendations to use Mintsoft's authoritative back-orders
-- when present, falling back to the live order_lines derivation.
...
AS $function$
  ...
  base AS (
    SELECT
      ...
      -- Prefer Mintsoft's authoritative figure; fall back to live derivation.
    ...
    LEFT JOIN sku_prefixes sp ON sp.prefix = upper(split_part(pc.sku, CASE WHEN pc.sku LIKE '%/%' THEN '/' ELSE '-' END, 1))
