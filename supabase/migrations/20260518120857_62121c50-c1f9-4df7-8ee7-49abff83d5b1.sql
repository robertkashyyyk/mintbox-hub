...
-- 1. Add box_quantity column to products_cache
ALTER TABLE public.products_cache
ADD COLUMN IF NOT EXISTS box_quantity integer NOT NULL DEFAULT 1;

COMMENT ON COLUMN public.products_cache.box_quantity IS
'Minimum order multiple — purchase quantities are rounded up to a multiple of this value. Default 1 means no constraint.';

CREATE INDEX IF NOT EXISTS idx_products_cache_box_quantity
  ON public.products_cache (box_quantity)
  WHERE box_quantity > 1;

-- 2. Drop old function (return type is changing — new columns added)
DROP FUNCTION IF EXISTS public.get_buy_recommendations(uuid, uuid, boolean);

CREATE FUNCTION public.get_buy_recommendations(
  p_supplier_id uuid DEFAULT NULL::uuid,
  p_brand_id uuid DEFAULT NULL::uuid,
  p_include_pending boolean DEFAULT false
)
RETURNS TABLE(
  sku text, product_name text, brand_id uuid, brand_name text,
  supplier_id uuid, supplier_name text,
  current_stock numeric, on_order numeric, back_orders numeric, low_stock_alert numeric,
  unit_cost numeric, required_qty numeric, raw_required_qty numeric, box_quantity integer,
  pending_po_qty numeric, pending_po_id uuid, sales_4w numeric, status text
)
LANGUAGE sql
...
-- 3. Recreate the summary helper that depends on this function (return shape unchanged)
CREATE OR REPLACE FUNCTION public.get_buy_recommendations_summary()
RETURNS TABLE(
  recommended_count bigint, pending_po_count bigint,
  total_required_qty numeric, total_required_cost numeric,
