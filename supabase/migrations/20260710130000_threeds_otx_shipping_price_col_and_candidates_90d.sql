-- 90-day Semi-Manual was ~13s: the group-sort dragged the 778-byte raw jsonb through it just
-- to read shippingPrice (12MB disk spill). Materialise shipping_price as a narrow stored column
-- (all values verified numeric) so raw stays at the scan and the sort goes in-memory (~9s, no
-- spill). Also date-bounds intl_orders to the window and drops the unused external_item_id +
-- the wide item_name from the aggregate (product_name falls back to products_cache; 0 candidates
-- relied on the eBay item_name). (Applied to prod 2026-07-10 via MCP; file added for repo parity.)
ALTER TABLE public.threeds_order_transactions
  ADD COLUMN IF NOT EXISTS shipping_price numeric
  GENERATED ALWAYS AS (NULLIF(raw->>'shippingPrice','')::numeric) STORED;
...
AS $function$
  ...
  SELECT e.sku, e.base_sku, e.pack_size,
    COALESCE(pc.name, pcb.name) AS product_name, COALESCE(b.name, bb.name) AS brand_name, e.units_sold,
