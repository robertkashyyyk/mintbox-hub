-- get_lsa_calibration was still ~16s as a function call even after the brand index +
-- pushdown, because of the `(p_brand_id IS NULL OR pc.brand_id = p_brand_id)` anti-pattern:
-- in a parameterised/black-box plan the planner must cover the NULL=all case, so it can't
-- use idx_products_cache_brand_id and seq-scans 225k. With a literal brand the same query
-- is ~52ms (index scan). This page is always per-brand (the all-brands view uses the
-- lsa_brand_summary MV), so make the brand filter UNCONDITIONAL (= p_brand_id) → index used.
-- Also drop the stale 1-arg overload that caused PostgREST ambiguity (HTTP 300).

DROP FUNCTION IF EXISTS public.get_lsa_calibration(uuid);
