-- Migration: Fix buy recommendations — DIRT filter, LSA threshold, and performance indexes
-- Applied: 2026-05-28
...
VALUES ('lsa.min_threshold', '1')
ON CONFLICT (key) DO NOTHING;

-- 3. Rebuild get_buy_recommendations with DIRT filter + use the new indexes
...
AS $function$
  ...
  bo_live AS (
    -- Uses idx_order_lines_backorder_sku partial index
  ...
  s4w AS (
    -- Uses idx_order_lines_date_sku index
  ...
  base AS (
    ...
    WHERE COALESCE(pc.quarantined, false) = false
      ...
      AND pc.sku NOT LIKE 'DIRT%'        -- exclude DIRT SKUs
      AND (
