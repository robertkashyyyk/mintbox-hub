...
-- Function: live queue counts
CREATE OR REPLACE FUNCTION public.get_ops_queue_counts()
RETURNS TABLE(
  new_count bigint,
  awaiting_picking_count bigint,
  onbackorder_count bigint,
  despatched_today_count bigint
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT
    count(*) FILTER (WHERE order_status = 'NEW') as new_count,
    count(*) FILTER (WHERE order_status = 'AWAITINGPICKING') as awaiting_picking_count,
    count(*) FILTER (WHERE order_status = 'ONBACKORDER') as onbackorder_count,
    count(*) FILTER (WHERE order_status = 'DESPATCHED' AND last_status_change_at::date = CURRENT_DATE) as despatched_today_count
  FROM order_lines
  WHERE order_date >= '2026-01-01'::timestamptz;
$$;

-- Function: despatch performance for date range
CREATE OR REPLACE FUNCTION public.get_despatch_performance(from_date date, to_date date)
RETURNS TABLE(
  within_24h bigint,
  within_48h bigint,
  within_72h bigint,
  total_despatched bigint
)
LANGUAGE sql
STABLE
SECURITY DEFINER
