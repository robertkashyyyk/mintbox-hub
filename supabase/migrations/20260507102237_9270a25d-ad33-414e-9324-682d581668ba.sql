-- Fix 1: LSA below threshold should not contribute to required_qty
...
AS $function$
  ...
  base AS (
    SELECT
      ...
      -- Effective LSA: zero when at/under the threshold
    ...
    WHERE COALESCE(pc.quarantined, false) = false
      ...
      AND (COALESCE(pc.low_stock_alert_level, 0) > cfg.lsa_min OR COALESCE(bo.back_orders, 0) > 0)
  ),
  scored AS (
    SELECT *,
...
-- Fix 2: Track Mintsoft ASN references on POs
ALTER TABLE public.purchase_orders
  ADD COLUMN IF NOT EXISTS mintsoft_asn_reference text,
  ADD COLUMN IF NOT EXISTS mintsoft_send_error text,
  ADD COLUMN IF NOT EXISTS mintsoft_send_attempted_at timestamptz;