-- ============================================================================
-- Fix: the Unlisted readiness `has_category` (and ready_score) must honour a
-- per-SKU draft category (listing_drafts.ebay_category_id), not just the
-- category map — otherwise setting the category in the drawer never ticks the
-- table's "Cat" check. Mirrors get_listing_detail's COALESCE(draft, map).
-- ============================================================================
