-- ============================================================================
-- 20260630210000_amazon_fba_net_margin.sql
-- Close the loop: real net margin per SKU with the CHANNEL fee (referral) and
-- the FBA fee (fulfilment/courier) kept SEPARATE — both from Finances, not a
-- blended guess.
--
-- The economics aggregate S&T (avg sell price) + 68k financial_events
-- (referral + FBA fees) per catalogue base_sku. That's too heavy to recompute
-- on every page load, so it lives in a MATERIALIZED VIEW (refresh nightly /
-- after a finance pull). v_fba_replenishment then joins it cheaply, and the
-- public wrapper is rebuilt so the new columns are actually exposed.
-- ============================================================================

-- Per-base_sku economics: avg sell price + separate referral / FBA fees. -------
...
-- seller-SKU -> catalogue SKU (financial_events carry the Amazon seller-SKU)
sku_cat AS (
...
CREATE UNIQUE INDEX IF NOT EXISTS mv_sku_economics_pk ON amazon.mv_sku_economics (marketplace_id, base_sku);
...
-- Replenishment view: cost + separate channel/FBA fees + true net margin. ------
...
SELECT
    ...
    -- true net per unit = price - referral - FBA fee - cost
...
-- Rebuild the public wrapper so the new columns are exposed (SELECT * froze the
-- old column list at creation).
