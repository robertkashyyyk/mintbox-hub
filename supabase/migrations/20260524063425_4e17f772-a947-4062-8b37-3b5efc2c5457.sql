...
CREATE OR REPLACE FUNCTION public.get_stock_valuation_summary(
  ...
  p_exclude_remote boolean DEFAULT true
) RETURNS TABLE(
  total_skus bigint,
  total_units numeric,
  total_value numeric,
...
AS $$
  ...
  base AS (
    ...
    WHERE (NOT p_exclude_remote OR is_remote = false)
  ),
  cats AS (
  ...
  SELECT
    ...
    (SELECT count(*) FROM all_rows WHERE is_remote)::bigint,
    COALESCE((SELECT SUM(current_stock) FROM all_rows WHERE is_remote), 0),
    COALESCE((SELECT SUM(net_value) FROM all_rows WHERE is_remote), 0),
    COALESCE(
...
GRANT EXECUTE ON FUNCTION public.get_stock_valuation_summary(uuid, boolean, boolean) TO authenticated;