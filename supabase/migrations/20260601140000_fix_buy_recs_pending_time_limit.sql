-- Fix: get_buy_recommendations pending CTE has no time limit, causing SKUs from
-- old stuck POs (status='sent', no mintsoft_po_id) to be permanently suppressed.
-- Align pending CTE to the same 7-day lookback used by useSentPoSuppression hook.
...
AS $function$
  ...
  pending AS (
    ...
    -- POs stuck longer than that without a Mintsoft ID should not permanently
    -- suppress their SKUs from the buy recommendations list.
