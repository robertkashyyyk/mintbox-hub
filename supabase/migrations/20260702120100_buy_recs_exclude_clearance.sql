-- ============================================================================
-- Buy engine: never recommend a SKU that's on an active Sale/Liquidation.
--
-- Belt-and-braces companion to the LSA clearance ring-fence (20260702120000).
-- The LSA fix floors a clearance SKU's low_stock_alert_level so effective_lsa
-- becomes 0 and the buy engine stops recommending it — but that depends on the
-- auto-LSA cron having run (push lag), and a clearance item that generates a
-- customer backorder could still trip the (back_orders + effective_lsa) >
-- (stock + on_order) test. While we're deliberately CLEARING stock, we never
-- want a purchase-order suggestion for it, full stop. So exclude active
-- sale/liquidation SKUs from the candidate set outright.
--
-- Only added line vs 20260616190000 is the NOT EXISTS clearance predicate in the
-- `cand` WHERE; everything else is byte-identical (a strict subset of candidates,
-- so the perf-parity guarantees of that migration are preserved).
-- ============================================================================
...
AS $function$
  ...
  cand AS MATERIALIZED (
    ...
    WHERE COALESCE(pc.quarantined, false) = false
      ...
      -- don't reorder anything we're actively clearing (Sale/Liquidation).
      AND NOT EXISTS (
