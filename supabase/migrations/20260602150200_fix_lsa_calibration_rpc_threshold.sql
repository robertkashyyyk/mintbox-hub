...
AS $function$
  ...
  base AS (
    ...
    WHERE COALESCE(pc.quarantined, false) = false
      ...
      AND (COALESCE(pc.low_stock_alert_level, 0) > 0 OR COALESCE(sales.units, 0) > 0)
  ),
  scored AS (SELECT *, ROUND(weekly_velocity * base_multiplier)::numeric AS target_lsa FROM base)
...
CREATE OR REPLACE FUNCTION public.get_lsa_calibration(p_brand_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 1000, p_offset integer DEFAULT 0)
RETURNS TABLE(sku text, product_name text, brand_id uuid, brand_name text, supplier_id uuid, supplier_name text, current_stock numeric, current_lsa numeric, weekly_velocity numeric, base_multiplier numeric, target_lsa numeric, status text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $function$
  WITH cfg AS (
