...
// ── Listing generation (GTC import template) — builder lives in @/lib/gtc ──
function CreateListingsDialog({ rows, onClose }: { rows: Unlisted[]; onClose: () => void }) {
  ...
  async function generate() {
    ...
    try {
      ...
      const gtcRows: { data: ListingData; cfg: StoreCfg | undefined }[] = [];
      let missingCat = 0;
      for (const sid of storeIds) {
        const cfg = cfgByStore.get(sid);
        for (const d of ld) { if (!d.ebay_category_id) missingCat++; gtcRows.push({ data: d, cfg }); }
      }
      downloadCsv(`3d-gtc-listings-${new Date().toISOString().slice(0, 10)}.csv`, buildGtcCsv(gtcRows));
      toast.success(`Generated ${gtcRows.length} row(s)${missingCat ? ` — ${missingCat} missing a category` : ""}`);
      onClose();
