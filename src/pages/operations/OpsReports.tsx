...
const OpsReports = () => {
  ...
  const addSubscriber = useMutation({
    mutationFn: async ({ name, email }: { name: string; email: string }) => {
      const { error } = await supabase
        .from('ops_report_subscribers')
        .insert({ name, email, report_type: 'weekly' });
      
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ops-report-subscribers'] });
  ...
  const sendTestReport = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.functions.invoke('send-ops-weekly-report', {
        body: { test: true },
      });
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ops-report-logs'] });
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Manage weekly report subscribers and view send history
                </p>
              </div>
            </div>
            <Button 
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Weekly ops report sent Friday 16:45 UK time
              </CardDescription>
            </div>
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Add a new recipient for the weekly operations report.
                  </DialogDescription>
                </DialogHeader>
          ...
          <CardContent>
            {subscribersLoading ? (
                      ...
                      <TableCell>
                        <Switch
                          ...
                          onCheckedChange={(checked) => 
                            toggleSubscriber.mutate({ id: sub.id, enabled: checked })
                          }
                        />
                      </TableCell>
                      <TableCell>
              ...
              <p className="text-muted-foreground text-center py-8">
                No subscribers yet. Add subscribers to receive the weekly ops report.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Recent report sends and their status
            </CardDescription>
          </CardHeader>
          <CardContent>
            {logsLoading ? (
                ...
                <TableBody>
                  {logs.map((log) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>
                        {log.week_ending ? format(new Date(log.week_ending), 'dd MMM yyyy') : '—'}
                      </TableCell>
