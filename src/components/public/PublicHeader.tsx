    ...
    >
      <div className="container mx-auto px-4 flex items-center justify-between h-16 lg:h-18">
        ...
        <Link to="/" className="flex items-center gap-2 group">
          <div className="w-8 h-8 rounded bg-pd-accent flex items-center justify-center font-bold text-foreground text-sm">
            PD
          </div>
          ...
          {navLinks.map((l) => (
            <Link
              ...
              className={`text-sm font-medium tracking-wide uppercase transition-colors hover:text-pd-accent ${
                location.pathname === l.to ? "text-pd-accent" : "text-foreground/70"
              }`}
            >
              {l.label}
            </Link>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="lg:hidden bg-pd-charcoal/98 backdrop-blur-lg border-t border-foreground/5 pb-6 animate-in slide-in-from-top-2">
          <nav className="flex flex-col px-4 pt-2 gap-1">
            {navLinks.map((l) => (
              <Link
                ...
                className={`py-3 text-sm font-medium uppercase tracking-wide border-b border-foreground/5 transition-colors ${
                  location.pathname === l.to ? "text-pd-accent" : "text-foreground/70"
                }`}
              >
                {l.label}
